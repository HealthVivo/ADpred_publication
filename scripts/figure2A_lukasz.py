...
X_valid = np.hstack([np.sum(p[ptest] * LL_av, axis=1), 
                     np.sum(n[ntest] * LL_av, axis=1)])
y_valid = np.hstack([np.ones(len(ptest)), np.zeros(len(ntest))])
...
plt.fill(b0,a0, alpha=0.4, c='orange', label='AD negative')
plt.fill(b1,a1, alpha=0.5, c='skyblue', label='AD positive')

plt.plot(b0,a0, alpha=1, c='orange')
plt.plot(b1,a1, alpha=1, c='skyblue')

plt.xlabel('log-likelihood score',  fontsize=18)
