...
peaks = []  # highest adpred scores along the sequence
peak = 0 # variable used to find the peaks
...
for i in range(len(ohe)-30):
    ...
    if adpred>=0.8:
        if adpred > adpred_complete_sequence[i-1]:
            peak = adpred
        peaks.append(n-1)

heatmaps = {}
for i in peaks:
    a = mut_analysis(sequence[i:i+30], adpred_complete_sequence[i]).T
    heatmaps[sequence[i:i+30]] = a

results = {
    'adpred_profile_protein': adpred_complete_sequence,
    'mutanalysis': heatmaps
}
