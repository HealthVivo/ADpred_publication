...
Xvalid = np.vstack([np.hstack(get_aa_freq_pos(i)).T for i in np.hstack([positives[ptest,0], negatives[ntest,0]])])
yvalid = np.hstack([np.ones(len(ptest)), np.zeros(len(ntest))])
...
sns.set_style('ticks')
sns.heatmap(model.coef_[0].reshape(30,20), cmap='Spectral_r', ax=ax)
