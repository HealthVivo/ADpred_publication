...
def store_data_numpy(arr, TYPE='int8', path=analysis_home+'/temporary/'):
    '''
...
def subsample_negatives(list_of_arrs, sampleSize):
    ...
    return [arr[np.random.randint(0,len(arr),N_of_each_arr)] for arr in list_of_arrs]
...
def epoch_generator(n_train, p_train, batch_size=100):
    ...
    # dimensions of n_dim correspond to (#npy-files, #samples/file) of negative set
    # np.random.seed(0)
    n_idx = np.random.permutation( np.arange(n_train.shape[0]) )[:p_train.shape[0]]

    # Divide the negative set into parts that are equivalent to the number of samples of the positive
    negat = n_train[n_idx]

    # join positive and negatives
    X = np.vstack([negat, p_train])
    X = X.reshape(np.insert(X.shape[:],3,1))
    y = np.hstack([np.zeros(negat.shape[0]), np.ones(p_train.shape[0])])
    idx = np.random.permutation(np.arange(y.shape[0]))
...
def charge(seq):
    '''
        calculates the 
    '''
    posit = seq.count('R') + seq.count('K') + seq.count('H') *0.5
    negat = seq.count('E') + seq.count('D')
    return posit - negat

charge = np.frompyfunc(charge, 1,1)
...
def scrumble_index(X,y,seed=42):
    if len(X) != len(y):
        print('X and y have different numbber of samples')
        return
...
def get_percentiles(df, results, feature='iupred', lenCutoff=15):
    ...
    for i in TADs_idx:
        ...
        if start>50: pre50.append( np.hstack(df.loc[i,feature][start-50:start]).astype(float) )
        tad.append( np.hstack(df.loc[i,feature][start:end]).astype(float) )
        if end+50<END: pos50.append( np.hstack(df.loc[i,feature][end:end+50]).astype(float) )
    ...
    for i in tad:
        if len(i)==5: t.append(i)
            ...
            # if longer than 5 residues, get the central 5 residues
            b = int(len(i)/2)-2
            t.append(i[b:b+5])


    _25 = np.hstack([np.percentile(np.vstack(i),25, axis=0) for i in [pre50,t,pos50]])
    _50 = np.hstack([np.percentile(np.vstack(i),50, axis=0) for i in [pre50,t,pos50]])
    _75 = np.hstack([np.percentile(np.vstack(i),75, axis=0) for i in [pre50,t,pos50]])
...
def read_psipred_results(fileName):
    '''
    ...
    while True:
        try:
            k,v, v2 = next(f), next(f), next(f)
            k = k.strip()[1:]
            results_ss[k] = v.strip().split(',')[:-1]
            results_ss2[k] = v2.strip().split(',')[:-1]
        except StopIteration:
            break
    f.close()
    return results_ss, results_ss2
...
def accuracy(x,y, threshold=0.5):
    ...
    '''
    if y.dtype != int or x.shape[0]!=y.shape[0]: 
        return "y should be int and x,y same length"
    
    return np.sum((x>=threshold)==y) / x.shape[0]